document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (window.ethereum) {
        ...
        async function updateStats() {
            ...
            const userTotalDividendsClaimed = await contract.methods.userDividendsClaimed(userAccount).call();

            // Calcular la cantidad exacta de BNB que le corresponde al usuario como dividendos hoy
            const userSharePercentage = userDeposits / totalTreasuryPool;
            const dividendsAvailableToday = totalDividendsPool;
            ...
            document.getElementById('user-total-dividends-claimed').innerText = web3.utils.fromWei(userTotalDividendsClaimed, 'ether');
        }
